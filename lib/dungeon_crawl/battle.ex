defmodule DungeonCrawl.Battle do
  ...
  def fight(
        %Character{hit_points: hit_points_a} = char_a,
        %Character{hit_points: hit_points_b} = char_b
      )
  ...
  defp attack(%Character{hit_points: hit_points_a}, character_b) when hit_points_a == 0,
    do: character_b
  ...
  defp receive_message(character = %{name: "You"}, damage) do
    "You receive #{damage} damage. Current HP: #{character.hit_points}."
  end
